...
class GoogleSettings(BaseSettings):
    ...
    api_key: str = Field(..., min_length=1, description="Google API key for Custom Search JSON API")
    search_engine_id: str = Field(..., min_length=1, description="Google CX engine identifier")
    max_results_per_query: int = Field(default=10, ge=1, le=50)
    request_timeout_seconds: float = Field(default=15.0, ge=1.0, le=60.0)

    model_config = SettingsConfigDict(env_prefix="GOOGLE_", str_strip_whitespace=True, case_sensitive=False)


class DuckDuckGoSettings(BaseSettings):
    """DuckDuckGo search provider configuration.

    All fields optional. Defaults:
        DUCKDUCKGO_MAX_RESULTS_PER_QUERY=10
        DUCKDUCKGO_REQUEST_TIMEOUT_SECONDS=15.0
    """

    max_results_per_query: int = Field(default=10, ge=1, le=100)
    request_timeout_seconds: float = Field(default=15.0, ge=1.0, le=60.0)

    model_config = SettingsConfigDict(env_prefix="DUCKDUCKGO_", case_sensitive=False)
...
class DiscoverySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="DISCOVERY_", case_sensitive=False)
...
class Settings(BaseSettings):
    ...
    duckduckgo: DuckDuckGoSettings = Field(default_factory=DuckDuckGoSettings)
    ...
    def model_validate(cls, obj, *args, **kwargs):
        """Validate settings while preserving BaseSettings environment loading.

        Pydantic's BaseSettings.model_validate validates the supplied object
        directly, which bypasses the settings source pipeline. The tests exercise
        Settings.model_validate({}) as the canonical construction path, so we
        route mapping inputs through the normal constructor here.
        """
        if obj is None:
            return cls()
