...
_HTML_TAGS: Final[re.Pattern[str]] = re.compile(
    r"<(script|style|noscript|iframe|svg|math)[^>]*>.*?"  # block tags with content
    r"</\1>|"
    r"<[a-zA-Z][^>]*>"  # all other tags
    ,
...
_PARAGRAPH_BOUNDARY: Final[re.Pattern[str]] = re.compile(r"\n{2,}|\r\n{2,}")
...
def strip_html(raw_html: str) -> str:
    ...
    'Hello & World'
    """
    # 1. Decode HTML entities first (before tag removal so & → & survives)
    text = _HTML_ENTITIES.sub(lambda m: html.unescape(m.group(0)), raw_html)
    # 2. Remove block+inline HTML tags
...
def truncate(text: str, max_length: int = 300, suffix: str = "...") -> str:
    ...
    if len(text) <= max_length:
        return text
    # Ensure we don't truncate in the middle of a multi-byte char
